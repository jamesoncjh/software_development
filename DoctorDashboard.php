...
<body>
                ...
                <div class="collapse navbar-collapse" id="navbarCollapse">
                    <div class="navbar-nav ms-auto py-0">
                        <?php if (isset($_SESSION['role']) && $_SESSION['role']=="doctor") { ?>
                            <a href="DoctorDashboard.php" class="nav-item nav-link active">Dashboard</a>
                        <?php }else if (isset($_SESSION['role']) && $_SESSION['role']=="admin"){ ?>
                            <a href="ManageUser.php" class="nav-item nav-link">User</a>
                        <?php }else{ ?>
                            <a href="Home.php" class="nav-item nav-link">Home</a>
                        <?php } ?>
                        <a href="CheckDiseases.php" class="nav-item nav-link">
                            <?php echo isset($_SESSION['role']) && $_SESSION['role'] == 'doctor' || $_SESSION['role'] == 'admin' ? "Article Diseases" : 'Check Diseases'; ?>
                        </a>
    ...
    <?php
        ...
        // Query the database to get the doctor's articles that have comments
        $sql = "SELECT DISTINCT d.disease_id, d.disease_name, c.comment_content, c.comment_timestamp FROM diseases d JOIN comments c ON d.disease_id = c.disease_id WHERE d.user_id = '$user_id'  ORDER BY c.comment_timestamp DESC";
        $result = $mysqli->query($sql);
        
        // Display the articles in a table
        // echo '<table class="table">';
        // echo '<thead><tr><th>Article ID</th><th>Title</th></tr></thead>';
        // echo '<tbody>';
        // while($row = mysqli_fetch_assoc($result)) {
        //     echo '<tr><td>' . $row['disease_id'] . '</td><td>' . $row['disease_name'] . '</td></tr>';
        // }
        // echo '</tbody>';
        // echo '</table>';
    ?>
    ...
    <table class="table mt-5" id="users-table">
        <thead>
            ...
            <th class="col-2"> Comment Date </th> 
        ...
        <tbody>
            ...
            <tr class="text-center">
                ...
                <td class="col-2"><?php echo $row['comment_content']; ?></td>
                <td class="col-2"><?php echo $row['comment_timestamp']; ?></td>
